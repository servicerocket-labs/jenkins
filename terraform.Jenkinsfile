...
node {
  ...
  withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: "${AWS_CREDS}", usernameVariable: 'ak', passwordVariable: 'sk']]) {
    ...
    def run = "docker run --rm -u `id -u jenkins` -v ${wd}:${td} -w=${td} -e AWS_ACCESS_KEY_ID=${env.ak} -e AWS_SECRET_ACCESS_KEY=${env.sk} --entrypoint=/go/bin/terraform hashicorp/terraform:full"
